...
class BairroEuclid extends StatefulWidget {
  const BairroEuclid({Key key}) : super(key: key);

  @override
  _BairroEuclidState createState() => _BairroEuclidState();
}

class _BairroEuclidState extends State<BairroEuclid> {
  ...
  void didChangeDependencies() {
    ...
    adState.initialization.then((status) {
      setState(() {
        _banner = BannerAd(
          adUnitId: adState.bannerAdUnitId7,
          size: AdSize.banner,
          request: AdRequest(),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        children: [
          NameBairros(text: "Euclides/ Solo",),
          Divider(),
          ContainerObs(),
          Center(
            child:
